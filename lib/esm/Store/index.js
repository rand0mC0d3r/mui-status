...
function StatusProvider(_a) {
    ...
    var _d = useState([]), snackbar = _d[0], setSnackbar = _d[1];
    var _e = useState(initialSettings), settings = _e[0], setSettings = _e[1];
    ...
    useEffect(function () {
        if (settings.debug) {
            console.log('mui-status-store:', __assign(__assign({}, settings), { status: status, snackbar: snackbar }));
        }
